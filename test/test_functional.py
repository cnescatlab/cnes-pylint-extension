"""Functional full-module tests for additional PyLint checkers

Copied from PyLint project.
"""
import csv
import collections
import io
...
def parse_expected_output(stream):
    return [OutputLine.from_csv(row) for row in csv.reader(stream, 'test', quotechar="|")]
...
class LintModuleTest(unittest.TestCase):
    ...
    def __init__(self, test_file):
        ...
        plugins_path = os.path.join(os.getcwd(), os.pardir, 'checkers')
        utils.register_plugins(self._linter, plugins_path)
        # Reduce max mccabe numbers to allow checking the rule with a reasonable
        # number of lines of code
        self._linter.global_set_option('max-mccabe-number', 10)
        self._linter.global_set_option('max-simplified-mccabe-number', 10)
        # force the detection of all the missing parameters (docstrings with a
        # mere description won't be tolerated)
        self._linter.global_set_option('accept-no-param-doc', False)
